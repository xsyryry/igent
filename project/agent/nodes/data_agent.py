"""Data collection specialist agent node.

The main ReAct controller chooses data actions. This specialist owns execution
of data/PDF/crawler actions and returns compact handoff events through state.
"""
...
DATA_AGENT_TOOLS = {"data", "cambridge_crawler", "question_pdf", "export_question_pdf"}


def is_data_agent_tool(tool_name: str) -> bool:
    return tool_name in DATA_AGENT_TOOLS


@trace_node("data_agent")
def data_agent_node(state: AgentState) -> dict[str, Any]:
    """Execute data-domain actions selected by the supervisor."""
...
def _matching_tool_calls(state: AgentState) -> list[dict[str, Any]]:
    ...
    return [
        call
        for call in calls
        if isinstance(call, dict) and is_data_agent_tool(str(call.get("tool_name") or ""))
    ]
